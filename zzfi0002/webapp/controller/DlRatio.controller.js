sap.ui.define([
	...
	var excuteSure = true,
		iSuccess = 0,
		iFailed = 0;
	return BaseController.extend("zzfi0002.controller.DlRatio", {
		formatter: formatter,
		/**
		...
		onBeforeRebindTable: function (oEvent) {
			...
			var sCycleID = '';
			if (this.byId("smartFilterDlRatio")) {
				sCycleID = this.byId("smartFilterDlRatio").getFilterData().Zzcycle;
			}
		 ...
		 * 
			...
			// 2024/03/22 yahui add for Jira [SAP-981];
			if ( oKey.CycleID.items.length > 1  ){
				MessageBox.error(this.getResourceBundle().getText("Multiple Cycle"));
				return;
			}
        */
	        // 2024/03/22 yahui add for Jira [SAP-981];
			var cycleString = ''; // 将界面选中的CycleId拼接上逗号SaveDlRatio时callFunction传给后端去处理；
			for (var i = 0; i < oKey.CycleID.items.length; i++) {
				cycleString += oKey.CycleID.items[i].key;
				if (i < oKey.CycleID.items.length - 1) {
					cycleString += ','; // 在除最后一个CycleId外，添加逗号分隔符
				}
			}
			...
			this.getModel().callFunction("/SaveDlRatio", {
				method: "POST",
				urlParameters: {
